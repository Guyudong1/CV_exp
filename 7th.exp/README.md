...
本次实验基于ResNet-18官方网络架构及其预训练权重，以MNIST数据集为分类任务构建分类模型。实验通过加载预训练的ResNet-18模型，旨在加深对残差网络结构特点及迁移学习基本原理的理解。
...
通过本次实验，我对残差网络的整体结构有了更直观的认识，更加清楚地理解 ResNet的设计思路。

在模型结构方面，通过summary打印出的网络结构可以看出，ResNet-18由多层卷积和多个残差块组成。虽然结构中没有直接显示跳跃连接，但残差连接实际封装在BasicBlock内部。通过这种残差结构，网络在不断加深的同时仍然能够保持较好的训练效果，这也解释了为什么通道数逐步增大、特征图尺寸逐步减小，但模型依然可以稳定训练。
在训练结果上，使用预训练权重的ResNet-18在少训练次数和小数据下就能取得较高的分类准确率，整体表现比较稳定。后期准确率出现轻微波动，主要是由于训练数据是随机抽取的一部分，这是正常现象。
通过混淆矩阵可以发现大多数类别都能被正确分类，只有少数外观较相似的类别之间存在一定的误判情况，整体分类效果是比较理想的。
在迁移学习对比实验中，有无预训练权重的差异非常明显。使用预训练权重的模型在训练初期就有较好的性能，而不使用预训练权重的模型收敛较慢、准确率较低。这说明迁移学习能够充分利用在大规模数据集上学到的特征，在小数据集任务中具有明显优势。
总体来看，本次实验让我对ResNet网络结构和迁移学习方法有了更清楚的理解，也提升了自己训练深度学习模型的实际能力，为后续相关实验和课程学习打下了基础。
