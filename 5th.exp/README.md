...
**2. 测试阶段**
每个epoch训练结束后：先model.eval()：进入测试模式，关闭梯度计算：torch.no_grad()，因为在测试过程中，我们只需要利用模型进行前向传播（forward） 来得到预测结果，而不需要反向传播去更新参数。所以不需要梯度计算。前向计算得到输出，计算测试损失，然后预测类别pred = out.max(dim=1)[1]，最后统计预测正确的数量，用于计算测试准确率。
...
**通过损失值曲线和准确率曲线图，也可以看出模型收敛良好，未出现明显过拟合，并且性能优秀。**
